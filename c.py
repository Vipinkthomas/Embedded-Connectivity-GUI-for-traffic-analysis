...
class PacketAnalyser(Winforms.Form):
                ...
                if msg.get_ipv4_layer().ipv4_header.ip_address_destination == self.myIp:
                    ...
                    for i in msg.get_udp_layer().payload[:5]:
                        ...
                        if len(hex(int(i))) <= 3:
                            ...
                            x+=hex(int(i))[2:] + " "
                    self.richTextBox.Text += "{0:<29}{1:<25}{2}".format(msg.get_ipv4_layer().ipv4_header.ip_address_source,self.myIp,x) +"\r\n"
            ...
            try:
                ...
                if msg.mac_address_destination == self.myMac:
                    ...
                    for i in msg.payload[:5]:
                        x+=hex(int(i))[2:] + " "
                    self.richTextBox.Text += "{0:<29}{1:<25}{2}".format(msg.mac_address_source, self.myMac,x) +"\r\n"
    ...
    def sendMacFrames(self):
        ...
        if self.onePacket.Checked:
            g_ethernet_msg.send()
            self.richTextBox.Text += "{0:<29}{1:<25}{2}".format(self.myMac,self.destinationMacIp.Text,self.payload.Text) +"\r\n"
    ...
    def sendIpPackets(self):
        ...
        if self.onePacket.Checked:
            udp_packet.send()
            self.richTextBox.Text += "{0:<29}{1:<25}{2}".format(self.myIp,self.destinationMacIp.Text,self.payload.Text) +"\r\n"
